#-------------------------------------------------------------#
#'   All complete threads 2026 syntax  AKA
#'   current:      process_2026_full.R
#'    no longer:   full_threads_syntax.R   located in OU One Drive for welser
#'   _R_proj_1drive
#'   link to codebook
#'   
#'   https://docs.google.com/spreadsheets/d/16qqdn0GaxAf57hi5uTyzeeHezS8altPlnRH0C7RunLA/edit?gid=0#gid=0
#'   
#   load
#   Load packages----
...
library(googlesheets4)   #  blah blah
library(tidyverse)
library(janitor)
library(data.table)
...
edd <- fread("all_complete_threads_2026_rt.csv")
...
edd$random_uniform <- runif(n = nrow(edd), min = 0, max = 100)

edd_filtered <- edd %>% 
  filter(t_subreddit %in% c("AskAcademia", 
                            "college", "education", "GenZ", 
                            "GradSchool", "Parenting", "Professors", 
                            "socialskills", "Teachers", "teenagers"))


edd_combined <- edd_filtered %>% 
  filter( date_thread >= as.Date("2025-01-01"),
          random_uniform > 80)

fwrite(edd_combined, "edd_combined.csv")

fwrite(edd, "all_threads26_with_27indices.csv")


plot(edd$feed_socials_iln, edd$ai_iln)




# Example 4: Filter for marketplace and gig economy subreddits


platforms <- edd %>%
  filter(t_subreddit %in% c("AirBnB", "amazonprime", "craigslist", "doordash", 
                            "doordash_drivers", "Ebay", "eBaySellerAdvice", 
                            "eBaySellers", "enshittification", "Etsy", 
                            "EtsySellers", "FacebookMarketplace", 
                            "LinkedInLunatics", "Lyft", "lyftdrivers", 
                            "onlyfansadvice",
                            "Pinterest", "uberdrivers", "UberEatsDrivers"))


names(platforms)
table(platforms$t_subreddit)

# AirBnB         amazonprime          craigslist            doordash 
# 481                 494                  65                 497 
# doordash_drivers                Ebay    eBaySellerAdvice         eBaySellers 
# 494                 493                 462                 444 
# enshittification                Etsy         EtsySellers FacebookMarketplace 
# 403                 462                 469                 494 
# LinkedInLunatics                Lyft         lyftdrivers      onlyfansadvice 
# 496                 476                 492                 483 
# Pinterest         uberdrivers     UberEatsDrivers 
# 462                 496                 494 


fwrite(platforms, "platform.csv")


#   all subreddits

#	academia  AcademicPhilosophy      AdultEducation		
#	965                  15                  10		
#	AgentsOfAI              AirBnB         amazonprime		
#	392                 481                 494		
#	antiai         AskAcademia             ChatGPT		
#	498                1172                 497		
#	college          craigslist   cscareerquestions		
#	1423                  65                 497		
#	cscareerquestionsEU            doordash    doordash_drivers		
#	17                 497                 494		
#	Ebay    eBaySellerAdvice         eBaySellers		
#	493                 462                 444		
#	education    enshittification                Etsy		
#	436                 403                 462		
#	EtsySellers     Experiencddevs FacebookMarketplace		
#	469                 498                 494		
#	GenAlpha                GenX                GenZ		
#	495                 497                 497		
#	GradSchool     highereducation      jobsearchhacks		
#	1181                 134                 110		
#	LinkedInLunatics                Lyft         lyftdrivers		
#	496                 476                 492		
#	millenials      onlyfansadvice              OpenAI		
#	441                 483                 498		
#	Parenting           Pinterest          Professors		
#	497                 462                1450		
#	recruitinghell        socialskills            Teachers		
#	495                 485                1498		
#	teenagers         uberdrivers     UberEatsDrivers		
# 498                 496                 494 
